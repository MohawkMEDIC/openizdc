    ...
    public class RemoteSynchronizationService : ISynchronizationService, IDaemonService
    {
        ...
        public int Pull(Type modelType, NameValueCollection filter)
        {
            ...
            catch (Exception e)
            {
                ...
                var alertService = ApplicationContext.Current.GetService<IAlertService>();
                alertService?.BroadcastAlert(new AlertMessage(this.m_devicePrincipal?.Identity.Name ?? "System", "ALL", Strings.locale_downloadError, String.Format(Strings.locale_downloadErrorBody, e), AlertMessageFlags.Alert));
